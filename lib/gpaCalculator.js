...
export const GRADE_POINT_MAP = {
  "A": { points: 4.00, remarks: "Exceptional / Outstanding" },
  "A-": { points: 3.67, remarks: "Excellent" },
  "B+": { points: 3.33, remarks: "Very Good" },
  "B": { points: 3.00, remarks: "Good" },
  "B-": { points: 2.67, remarks: "Above Average" },
  "C+": { points: 2.33, remarks: "Average" },
  "C": { points: 2.00, remarks: "Satisfactory" },
  "C-": { points: 1.67, remarks: "Below Average" },
  "D": { points: 1.00, remarks: "Pass" },
  "F": { points: 0.00, remarks: "Fail" }
};
...
export function computeCourseResult(course) {
  const isNonCredit = course.isNonCredit === true;
  ...
  // Use explicit course grade if provided (e.g. from official university marksheet)
  let grade = course.grade;
  let points = 0;
  let remarks = "";
  if (grade && GRADE_POINT_MAP[grade.toUpperCase()]) {
    grade = grade.toUpperCase();
    points = GRADE_POINT_MAP[grade].points;
    remarks = GRADE_POINT_MAP[grade].remarks;
  } else {
    const calcGrade = getGradeFromPercentage(percentage);
    grade = calcGrade.grade;
    points = calcGrade.points;
    remarks = calcGrade.remarks;
  }
...
export function calculateCumulativeGPA(allSemestersMap, customSGPAs = null) {
  ...
  for (const semNum of semNumbers) {
    ...
    if (customSGPAs && customSGPAs[semNum] !== undefined) {
      semData.sgpa = Number(customSGPAs[semNum]);
      semData.totalWeightedPoints = Math.round(semData.sgpa * semData.totalCredits * 100) / 100;
    }
